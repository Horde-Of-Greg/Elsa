...
| **Command Result Cache** | `cmd-run:<commandName>:<key>`         | `commandName` - Name of the command<br>`key` - The parse cache key                             | `Elsa:cmd-run:help:default`<br>`Elsa:cmd-run:tag:myTag` | Caches command execution results. TTL varies by command config.                      |
