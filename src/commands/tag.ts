...
export async function handleTag(message: Message, args: string[]): Promise<void> {
  if (args.length < 2) {
  ...
  const res = await query('SELECT * FROM tags WHERE tag_name=$1 AND subtag_name=$2', [tagName, subtagName]);
